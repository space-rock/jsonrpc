/**
 * Unit tests for genesis_config RPC method.
...
describe('genesis_config - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should have valid request and response schemas', () => {
      expect(hasValidSchemas('genesis_config')).toBe(true);
      expect(methodSchemas['genesis_config']?.request).toBeDefined();
      expect(methodSchemas['genesis_config']?.response).toBeDefined();
    });
  });
  ...
  describe('Request Validation', () => {
    it('should create valid request structure', () => {
      ...
      const request = createJsonRpcRequest('genesis_config', mockParams);

      expect(request.jsonrpc).toBe('2.0');
      expect(request.method).toBe('genesis_config');
    ...
    it('should generate and validate request with schema', () => {
      const request = generateRequest('genesis_config');
      ...
      const isValid = validateRequest('genesis_config', request);
    ...
    it('should fail validation for invalid request', () => {
      ...
      const isValid = validateRequest('genesis_config', invalidRequest);
  ...
  describe('Response Validation', () => {
    ...
    it('should generate and validate response with schema', () => {
      const response = generateResponse('genesis_config', 'test-id');
      ...
      const isValid = validateResponse('genesis_config', response);
    ...
    it('should fail validation for invalid response', () => {
      ...
      const isValid = validateResponse('genesis_config', invalidResponse);
      expect(isValid).toBe(false);
    });
  });
