/**
 * Integration tests for genesis_config RPC method.
...
describe('genesis_config - Integration Tests', () => {
  ...
  describe('Mock RPC Tests', () => {
    it('should handle genesis_config request with mock response', async () => {
      ...
      server.use(
        http.post(endpoint, async ({ request }) => {
          ...
          expect(body?.method).toBe('genesis_config');
    ...
    it('should validate genesis_config request structure', async () => {
      const mockParams = generateDeterministicMockParams(
        'genesis_config',
        'test-2',
      );
      ...
      const isValid = validateRequest('genesis_config', validRequest);
      expect(isValid).toBe(true);
    });

    it('should handle genesis_config server error response', async () => {
      const mockParams = generateDeterministicMockParams(
        'genesis_config',
        'test-3',
      );
    ...
    it('should handle genesis_config network error', async () => {
      const mockParams = generateDeterministicMockParams(
        'genesis_config',
        'test-4',
      );
    ...
    it('should reject invalid genesis_config request', async () => {
    ...
    it('should handle genesis_config request with timeout simulation', async () => {
      // Pre-generate test data to ensure consistency
      const mockParams = generateDeterministicMockParams(
        'genesis_config',
        'test-6',
