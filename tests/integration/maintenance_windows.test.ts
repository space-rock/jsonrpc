/**
 * Integration tests for maintenance_windows RPC method.
...
describe('maintenance_windows - Integration Tests', () => {
  ...
  describe('Mock RPC Tests', () => {
    it('should handle maintenance_windows request with mock response', async () => {
      ...
      server.use(
        http.post(endpoint, async ({ request }) => {
          ...
          expect(body?.method).toBe('maintenance_windows');
    ...
    it('should validate maintenance_windows request structure', async () => {
      const mockParams = generateDeterministicMockParams(
        'maintenance_windows',
        'test-2',
      );
      ...
      const isValid = validateRequest('maintenance_windows', validRequest);
      expect(isValid).toBe(true);
    });

    it('should handle maintenance_windows server error response', async () => {
      const mockParams = generateDeterministicMockParams(
        'maintenance_windows',
        'test-3',
      );
    ...
    it('should handle maintenance_windows network error', async () => {
      const mockParams = generateDeterministicMockParams(
        'maintenance_windows',
        'test-4',
      );
    ...
    it('should reject invalid maintenance_windows request', async () => {
    ...
    it('should handle maintenance_windows request with timeout simulation', async () => {
      // Pre-generate test data to ensure consistency
      const mockParams = generateDeterministicMockParams(
        'maintenance_windows',
        'test-6',
