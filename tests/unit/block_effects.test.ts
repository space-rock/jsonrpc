/**
 * Unit tests for block_effects RPC method.
...
describe('block_effects - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should have valid request and response schemas', () => {
      expect(hasValidSchemas('block_effects')).toBe(true);
      expect(methodSchemas['block_effects']?.request).toBeDefined();
      expect(methodSchemas['block_effects']?.response).toBeDefined();
    });
  });
  ...
  describe('Request Validation', () => {
    it('should create valid request structure', () => {
      ...
      const request = createJsonRpcRequest('block_effects', mockParams);

      expect(request.jsonrpc).toBe('2.0');
      expect(request.method).toBe('block_effects');
    ...
    it('should generate and validate request with schema', () => {
      const request = generateRequest('block_effects');
      ...
      const isValid = validateRequest('block_effects', request);
    ...
    it('should fail validation for invalid request', () => {
      ...
      const isValid = validateRequest('block_effects', invalidRequest);
  ...
  describe('Response Validation', () => {
    ...
    it('should generate and validate response with schema', () => {
      const response = generateResponse('block_effects', 'test-id');
      ...
      const isValid = validateResponse('block_effects', response);
    ...
    it('should fail validation for invalid response', () => {
      ...
      const isValid = validateResponse('block_effects', invalidResponse);
      expect(isValid).toBe(false);
    });
  });
